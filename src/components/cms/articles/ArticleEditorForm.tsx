...
export function ArticleEditorForm({ articleId }: { articleId?: string }) {
  ...
  return (
    <>
      ...
      <div className="sticky bottom-4 z-30 mt-6 rounded-[1.5rem] bg-[#003f2d]/95 p-3 text-white shadow-2xl backdrop-blur-xl"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><button type="button" onClick={() => dirty ? setLeaveOpen(true) : router.push("/cms/articles")} className="rounded-full px-4 py-3 text-xs font-black text-white/70 hover:bg-white/10">← Kembali ke Daftar</button><div className="flex flex-wrap gap-2"><button type="button" disabled={saving} onClick={() => void preview()} className="rounded-full bg-white/10 px-4 py-3 text-xs font-black ring-1 ring-white/15 disabled:opacity-50">Preview</button>{currentId ? <button type="button" disabled={saving} onClick={() => void save("ARCHIVED")} className="rounded-full bg-white/10 px-4 py-3 text-xs font-black ring-1 ring-white/15 disabled:opacity-50">Arsipkan</button> : null}<button type="button" disabled={saving} onClick={() => void save("DRAFT")} className="rounded-full bg-white px-5 py-3 text-xs font-black text-[#006b3f] disabled:opacity-50">{saving ? "Menyimpan…" : "Simpan Draft"}</button><button type="button" disabled={saving} onClick={() => void save(form.status === "SCHEDULED" ? "SCHEDULED" : "PUBLISHED")} className="rounded-full bg-[#18c47c] px-5 py-3 text-xs font-black text-[#003f2d] disabled:opacity-50">{form.status === "SCHEDULED" ? "Jadwalkan" : "Terbitkan"}</button></div></div></div>
